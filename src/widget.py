        ...
        **kwargs : other keywords are passed to the plotting routine
        ...
        if y is not None:
            if np.ndim(x)!=np.ndim(y):
                raise ValueError('ERROR: x and y need to be both 1D or both 2D')
            
            if (np.ndim(x)==2) and (x.shape[1]!=y.shape[1]):
                raise ValueError('ERROR: x and y need to have same number of columns')
        ...
        #    
        if nx!=data.shape[1]:
            raise ValueError('ERROR: number of x points does not match the number of columns of the data array')
        if times is not None:
            if nt != len(times):
                raise ValueError('ERROR: len(times) does not match (number of rows)/ny of the data array')
        if y is None:
